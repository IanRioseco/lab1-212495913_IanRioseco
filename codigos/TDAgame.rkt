...
; Descripción: funcion que genera un historial
; Dom: game (game)
; Rec: game
; Tipo recursión: No aplica.
...
; Descripción: funcion que verifica si el estado actual del juego es empate
; Dom: game (game)
; Rec: boolean (#t si es empate, #f si no es empate)
; Tipo recursión: No aplica.
...
(define (game-player-set-move game player column)
  (let* ((p1 (car game))
    ...
    (if (not (eq? (get-player-id player) (get-player-id validar-turno)))
        (display "no es el turno del judador")
        (if (= piezas-restantes 0)
            (display "el jugador no tiene fichas para jugar")
